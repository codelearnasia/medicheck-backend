let MedicineController = {
    ...
    getSingleMedicine: (req, res, next) => {

    let medicine = req.params.medicineName;
    
        var medicines = [{
                medicineName: "Triptal",
                Pharmacy: [{
                    name: "Raffles Pharmacy",
                    quant: 50
                }, {
    ...
    var foundMedicine = medicines.find(function(element) {
      return element["medicineName"] == medicine;
    });

    if (Object.keys(foundMedicine).length === 0 && foundMedicine.constructor === Object){
        return res.send({"status":400,"message":"medicine not found in db"});
    }else{
        return res.send({"data":foundMedicine});        
    }
